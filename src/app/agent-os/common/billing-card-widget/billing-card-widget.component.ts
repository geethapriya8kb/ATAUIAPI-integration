...
export class BillingCardWidgetComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    const billingWidgetDetails = this.storeService.getBillingWidget();
    if (billingWidgetDetails) {
      this.data = billingWidgetDetails;
    } else {
  ...
  getBillingCardWidgetData(accountNumber) {
    ...
    this.groupservice.getdatafromAPI(accountNumber, this.cardName).subscribe({
      next: (resp) => {
        ...
        this.storeService.setBillingWidget(this.data);
      },
    });
  }
}
